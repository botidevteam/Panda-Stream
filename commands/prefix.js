module.exports = {
    ...
    run: async (call) => {
        ...
        Util.SQL_GetResult(Util.db_Model.servers, "ServerID", message, message.member)
            .then(results => {
                //console.log(results);
                /**
                ...
                if (!call.args[0] || call.args[0] === "help") {
                    ...
                    `\`\`\`Exemple: ${config.prefix}${call.cmd} set YourPrefix\`\`\`
                    `
                    )
                    */
                    embed_help.setDescription(
                        `${config.prefix}${call.cmd} **help**: This command\n` +
                        `${config.prefix}${call.cmd} **show**: Show the current prefix of this server\n` +
                        `${config.prefix}${call.cmd} **set**: Set the prefix of the bot on this server\n` +
                        `${config.prefix}${call.cmd} **reset**: Reset the prefix of the bot on this server`
                    )
                    ...
                    if (call.args[0] === "set") {
                        ...
                        call.bot.con.query(`UPDATE ${Util.db_Model.servers} SET ServerPrefix = '${config.prefix}' WHERE ServerID = '${message.guild.id}'`, (error, res) => {
                            if (error) { return message.channel.send(Util.errorMessage(error, "prefix")); }
                                ...
                                embed_message_good.setDescription(`Successfully reset the prefix of this server to \`${call.args[1]}\``)
                                message.channel.send(embed_message_good);
                            }
                        })
                    }
